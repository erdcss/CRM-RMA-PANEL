...
export function registerAuthRoutes(app:Express){
 app.post("/api/auth/login",async(req,res)=>{try{const email=String(req.body?.email||"").trim().toLowerCase(),password=String(req.body?.password||"");const r=await q("SELECT * FROM auth_users WHERE email=$1",[email]);const row=r.rows[0];if(!row||!row.is_active||!verifyPassword(password,row.password_hash))return res.status(401).json({error:"E-posta veya şifre hatalı"});const token=await createSession(row.id);res.json({token,user:mapUser(row)});}catch(e){console.error(e);res.status(500).json({error:"Giriş yapılamadı"});}});
